...
class AuthAPI(pykern.quest.API):
    ...
    def pykern_http_validate_client(self, auth_args: AuthArgs):
        ...
        """
        if auth_args.client_id is None or auth_args.client_id not in self.pykern_http_clients:
            for _ in range(5):
                ...
                raise AssertionError("unabled to generate unique client id")
        return AuthResult(client_id=auth_args.client_id)

    def api_pykern_http_auth(self, auth_args: AuthArgs):
        ...
        if auth_args.version != self.PYKERN_HTTP_VERSION:
            raise APICallError(f"invalid version={auth_args.version}, expected={self.PYKERN_HTTP_VERSION}")
        if self.PYKERN_HTTP_TOKEN is not None and self.PYKERN_HTTP_TOKEN != auth_args.token:
            raise APIForbidden()
...
class HTTPClient:
    ...
    async def connect(self, auth_args):
        """Connect to the server

        Args:
            auth_args (PKDict): how to authenticate connection; may be AuthArgs or other PKDict
        Returns:
            AuthReply: status of connection
        """
        def _auth():
            try:
                self.client_id = self.call_api(AUTH_API_NAME, auth_args).client_id
...
def server_start(api_classes, attr_classes, http_config, coros=(), auth_op=None):
    ...
    _HTTPServer(l, api_classes, attr_classes, http_config, session_auth=())
    if coros:
        l.run(*coros)
    l.start()
