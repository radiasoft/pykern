...
def _version(base):
    ...
    if not _cfg:
        _cfg = pkconfig.init(no_version=(False, bool, "use now(datetime.timezone.utc)s version"))
    if _cfg.no_version:
...
def _version_from_datetime(value=None):
    ...
    return str(
        packaging.version.Version(
            (value or datetime.datetime.now(datetime.timezone.utc)).strftime("%Y%m%d.%H%M%S"),
        ),
    )
