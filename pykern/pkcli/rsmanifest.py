...
def add_code(name, version, uri, source_d, virtual_env=None, pyenv=None):
    ...
    if pyenv:
        assert not virtual_env, "only one of pyenv or virtual-env (DEPRECATED)"
    elif virtual_env:
        assert not pyenv, "only one of pyenv or virtual-env (DEPRECATED)"
        pyenv = virtual_env
    if not pyenv:
        pyenv = _NO_PYENV
    v = values.codes.get(pyenv) or pkcollections.Dict()
    v[name.lower()] = pkcollections.Dict(
        installed= pykern.pkcompat.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),
        name=name,
        source_d=source_d,
        uri=uri,
        version=version,
...
def read_all():
    ...
    assert u.version == c.version, "(user.version) {} != {} (container.version)".format(
        u.version, c.version
    )
